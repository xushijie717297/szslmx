<template>
    <div class="ModelSummaryPipe">
    <h5>管径参数汇总</h5>
    <div class="border">
      <div id="echartModelPipe" style="width: 9.36rem; height: 4.3rem"></div>
    </div>
  </div>
</template>
...
export default {
    ...
    methods: {
       GetMaterialInfo(id) {
      axios
        .post(UrlClass.homePage + "GetDiameterInfo", "", global.axiosHeaders)
        .then((response) => {
    ...
    mounted () {
      ...
      this.GetMaterialInfo("echartModelPipe");
      var that = this;
      var resizeTimer = null;
      window.onresize = function () {
        if (resizeTimer) clearTimeout(resizeTimer);
        resizeTimer = setTimeout(function () {
          that.GetMaterialInfo("echartModelPipe");
        }, 100);
      };
    }); 
    }
