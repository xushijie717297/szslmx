        ...
        <v-touch v-on:swipedown="go" v-on:swipeup="up">
          <div class="foot" v-if="foot">
            ...
            <div class="inside">
              <span>水质警报列表</span>
              <div>
                <div class="side" :class="{tab : change == index}" @click = 'clicked(index)' v-for="(item,index) in sideList" :key = 'index'>{{item}}</div>
              </div>
            </div>
          </div>
           </v-touch>
          <div v-for="(item,index) in sideContent" v-show="index==change" :key="index">
            <v-touch v-on:swipedown = "downn" v-on:swipeup = "upp">
              <ul style="border-bottom: 1px solid #1B314E;box-sizing: border-box;">
                <li v-for="(item,index) in who" :key='index' @click='tab(index)' :class="{active:index == num}">
                  <span>{{item.name}}</span>
                  <p>{{item.num}}</p>
                </li>
              </ul>
            ...
            <div class="swiper">
                    ...
                    <div class="swiper-r">
                      <div class="tabs" v-for="(item,index) in whole" :key="index" v-show = 'index==num'>
                        <!-- 纵向轮播图 -->
                        <vartical v-if="aginReset" :tabIndex="num" v-on:childByValue="getData" v-on:childByXy="getDataXy"></vartical>
                      </div>
                    </div>
                  </div>
          </div>
       </div>
       <div class="reductio" style="background: url(../../../static/image/button_quanping_01.png);" @click="reduction()"></div>
    </div>
    <!-- 曲线组件压缩 -->
...
<script>
import $ from 'jquery'
import diffuseBubbles from '../../components/arcgisforjsTest/DiffuseBubbles'
...
import vartical from '@/components/vartical'
  export default {
     ...
     data () {
        return {
          aginReset:true,//子组件是否显示
            ...
            num:0,
            change:0,//控制tab切换参数
            sideList:['浊度','余氯','PH'],//tab标题
            sideContent:['我是内容一','我是内容二','我是内容三'],
            // point:[
            //         {"pointX":'112595.09642798237',"pointY":'18184.751207988316'},
            //         {"pointX":'109123.75615196848',"pointY":'17549.749937985773'},
            //         {"pointX":'112044.76199398017',"pointY":'19751.08767399458'},
            //         {"pointX":'112087.09541198034',"pointY":'18481.0851339895'},
            //         {"pointX":'113314.76453398525',"pointY":'20682.422869998307'},
            //         {"pointX":'114881.10099999151',"pointY":'19497.087165993566'},
            //       ],
                  point : [
                  ...
                  str:[//顺时针渲染
                      ...
                      color:[[225, 0, 225,0.5],[0, 225, 0,1],[0, 0, 225,0.5],[255, 0, 0,1]]
        }
     },
     components:{
       vartical
     ...
     methods:{
       ...
       reduction(){
         this.view.extent = new this.arcgisAPI.Extent(91018.9439, 7611.9801, 142976.5999, 34599.5340, new this.arcgisAPI.SpatialReference({ wkid: 102113 }));
       },
       clicked(index){
         this.view.extent = new this.arcgisAPI.Extent(91018.9439, 7611.9801, 142976.5999, 34599.5340, new this.arcgisAPI.SpatialReference({ wkid: 102113 }));
         var page = 0;
    ...
    loadModules(['esri/Map',
         ...
         .then(([ArcGISMap, MapView,WebTileLayer,TileInfo,MapImageLayer,SpatialReference,Extent,Graphic,Home,esriConfig,Polygon,SimpleFillSymbol]) => {
           ...
           let webTileMap = new WebTileLayer("http://121.40.242.176:8098/Service1.svc/Rest/GMService/{col}/{row}/{level}/2/google", {tileInfo: wtl_tileInfo});
           map.add(webTileMap);
           var dynamicMapServiceLayer = new MapImageLayer("http://112.64.170.158:8888/ArcGIS/rest/services/ModelPipe_sz_BaseMap/MapServer");
           map.add(dynamicMapServiceLayer);
           var view=this.view = new MapView({
             container: "map",
             map: map,
             extent:  new Extent(91018.9439, 7611.9801, 142976.5999, 34599.5340, new SpatialReference({ wkid: 102113 })),
           ...
           this.view.on('click', function (event) {
                ...
                // this.view.ui.add(new Home({view: this.view}), "top-right");
                // debugger
           //      this.view.when(function(e){
           //        diffuseBubbles.initPopupEvent(self.view);
           //        var tempData = [];
           //        for (let i = 0; i < self.point.length; i++) {
           //          const obj = self.point[i];
           //          var tempObj = {
           //            id: i,
           //            type: obj.TYPE,
           //            isUp: obj.ISUP,
...
<style scoped="" lang="less">
  @r : 1rem/37.5;
 .waters /deep/ .esri-ui-corner-container{
    left: 0 !important;
    bottom: 0 !important;
    top: 50px !important;
    right: 5px !important;
  }
  .waters /deep/ .esri-widget--button{
    outline: none;
  }
//初始化按钮
  .reduction, .reductio{
